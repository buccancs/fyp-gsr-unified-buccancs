...
class FileTransferManager:
    ...
    def request_device_files(
        self, device_manager, device_id: str
        ...
        try:
            ...
            success = device_manager.send_command_to_device(
                device_id, "get_recorded_files"
            )
            if not success:
    ...
    def start_file_transfer(
        self, device_manager, device_id: str, destination_dir: str
    ) -> bool:
        ...
        try:
            ...
            self._notify_callbacks(
                device_id, "transfer_started", self.active_transfers[device_id]
            )
            return True
    ...
    def _transfer_files_thread(
        ...
        try:
            ...
            success = device_manager.send_command_to_device(
                device_id, "start_file_transfer", transfer_command
            )
            if not success:
            ...
            if device_id in self.active_transfers:
                ...
                self._notify_callbacks(
                    device_id, "transfer_failed", self.active_transfers[device_id]
                )
    ...
    def _setup_transfer_server(self, device_id: str, destination_dir: str):
        ...
        try:
            ...
            logger.info(
                f"File transfer server listening on port {self.transfer_port} for device {device_id}"
            )

            # Wait for device connection
            client_socket, client_address = server_socket.accept()
            logger.info(
                f"Device {device_id} connected for file transfer from {client_address}"
            )
    ...
    def _handle_file_transfer(
        self, device_id: str, client_socket: socket.socket, destination_dir: str
    ):
        ...
        try:
            ...
            while True:
                ...
                if header.get("type") == "file":
                    ...
                    logger.info(
                        f"Receiving file {filename} ({file_size} bytes) from device {device_id}"
                    )
                    ...
                    with open(file_path, "wb") as f:
                        while received_size < file_size:
                            ...
                            self._notify_callbacks(
                                device_id, "transfer_progress", transfer_info
                            )

                    if received_size == file_size:
                        transfer_info["transferred_files"] += 1
                        logger.info(
                            f"Successfully received file {filename} from device {device_id}"
                        )
                        ...
                        if (
                            ...
                            self.session_manager.add_device_file(
                                device_id, file_type, file_info
                            )
                    else:
                        logger.error(
                            f"Incomplete file transfer for {filename} from device {device_id}"
                        )
                    ...
                    transfer_info["duration"] = (
                        transfer_info["end_time"] - transfer_info["start_time"]
                    )
                    self._notify_callbacks(
                        device_id, "transfer_completed", transfer_info
                    )
                    break
    ...
    def cancel_transfer(self, device_id: str) -> bool:
        ...
        try:
            if device_id in self.active_transfers:
                ...
                self._notify_callbacks(
                    device_id, "transfer_cancelled", self.active_transfers[device_id]
                )
    ...
    def start_batch_transfer(
        self, device_manager, device_ids: List[str], destination_dir: str
    ) -> Dict[str, bool]:
        """Start file transfer for multiple devices."""
        results = {}
        for device_id in device_ids:
            results[device_id] = self.start_file_transfer(
                device_manager, device_id, destination_dir
            )
        return results
