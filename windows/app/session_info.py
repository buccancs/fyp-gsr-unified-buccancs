...
class DeviceInfo:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'DeviceInfo':
        ...
        device_info.files = [
            FileInfo.from_dict(f) for f in data.get(
                "files", [])]
        return device_info
...
class SessionInfo:
    ...
    def set_start_time(self, start_time: Optional[int] = None) -> None:
        ...
        """
        self.start_time = start_time if start_time is not None else int(
            time.time() * 1000)
    ...
    def set_end_time(self, end_time: Optional[int] = None) -> None:
        ...
        """
        self.end_time = end_time if end_time is not None else int(
            time.time() * 1000)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'SessionInfo':
        ...
        session.devices = [DeviceInfo.from_dict(
            d) for d in data.get("devices", [])]
        session.sync_events = [
            SyncEvent.from_dict(e) for e in data.get(
                "sync_events", [])]
