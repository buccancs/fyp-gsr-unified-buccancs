...
def calculate_time_offsets(
    ...
    for device in session.devices:
        ...
        if "start" in events_by_type:
            reference_start_time = find_event_time_for_device(
                events_by_type["start"], reference_device_id)
            device_start_time = find_event_time_for_device(
                events_by_type["start"], device.device_id)
        ...
        for event_type, events in events_by_type.items():
            reference_time = find_event_time_for_device(
                events, reference_device_id)
...
def convert_to_reference_time(
        local_timestamp: int,
        device_info: DeviceInfo) -> int:
    """
        ...
        if device_events:
            ...
            for event in device_events:
                ...
                for ref_event in session.sync_events:
                    if reference_device.device_id in ref_event.type and event.type.split(
                            '_')[0] == ref_event.type.split('_')[0]:
                        # Calculate drift after applying offset
                        adjusted_time = convert_to_reference_time(
            ...
            if drifts:
                ...
                device_stats["avg_drift_ms"] = sum(
                    abs(d) for d in drifts) / len(drifts)
                device_stats["drift_std_dev"] = (
                    sum((d - device_stats["avg_drift_ms"])**2 for d in drifts) / len(drifts))**0.5
