...
def perform_ntp_like_sync(ping_func, pong_handler, num_samples=5):
    ...
    for _ in range(num_samples):
        ...
        offset = calculate_adjusted_offset(
            local_send_time + one_way, remote_time, 0)
        offsets.append(offset)

    # Filter out samples with high RTT (optional)
    valid_samples = [(offsets[i], round_trip_times[i])
                     for i in range(len(offsets))]
    valid_samples.sort(key=lambda x: x[1])  # Sort by RTT

    # Use the median or average of the best samples
    if valid_samples:
        best_samples = valid_samples[:max(
            1, len(valid_samples) // 2)]  # Use best half
        final_offset = sum(sample[0]
                           for sample in best_samples) / len(best_samples)
