...
class PerformanceMonitor {
    ...
    interface PerformanceCallback {
        ...
        fun onPerformanceAlert(metric: String, value: Double, threshold: Double)
    }
     ...
     */
    fun recordDataThroughput(component: String, bytes: Long) {
    ...
    fun getMetrics(): Map<String, MetricData> {
        return metrics.mapValues { it.value.getMetricData() }
    }
    ...
    fun getMetric(name: String): MetricData? {
        return metrics[name]?.getMetricData()
    }

    /**
     * Set performance threshold for alerts.
     */
    fun setThreshold(metricName: String, threshold: Double) {
    ...
    fun getPerformanceSummary(): String {
        ...
        return buildString {
            ...
            metrics.forEach { (name, data) ->
                appendLine("$name: ${String.format("%.2f", data.current)} (avg: ${String.format("%.2f", data.average)}, max: ${String.format("%.2f", data.maximum)})")
            }
        }
    }
