...
class SystemTestingUtility(private val context: Context) {
    ...
    private suspend fun testDataSerialization(): TestResult {
        ...
        try {
            ...
            val gsrMatch = Math.abs(deserializedJson.getDouble("gsr_value") - (originalData["gsr_value"] as Double)) < 0.001
    ...
    fun getTestSummary(): String {
        ...
        return buildString {
            ...
            appendLine("Success Rate: ${if (total > 0) String.format("%.1f", (passed.toDouble() / total) * 100) else "0.0"}%")
