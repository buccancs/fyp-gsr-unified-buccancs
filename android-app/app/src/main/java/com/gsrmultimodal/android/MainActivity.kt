...
import com.gsrmultimodal.android.GsrHandler
...
class MainActivity : AppCompatActivity() {
    ...
    companion object {
        private const val TAG = "GSRMultimodal"
        private const val ACTION_USB_PERMISSION = "com.gsrmultimodal.android.USB_PERMISSION"
    ...
    private fun initializeHandlers() {
        // Initialize camera handler with window for sync markers
        ...
        cameraHandler.setRawFrameCallback(object : CameraHandler.RawFrameCallback {
            override fun onRawFrameReceived(frameData: ByteArray, width: Int, height: Int, timestamp: Long, frameNumber: Long) {
                ...
                if (frameNumber % 30 == 0L) { // Log every 30th frame to avoid spam
                    Log.d("MainActivity", "RGB raw frame received: ${frameData.size} bytes, ${width}x${height}, frame #${frameNumber}")
                }
            }
        ...
        thermalCameraHandler.setThermalCallback(object : ThermalCameraHandler.ThermalDataCallback {
            override fun onThermalFrameReceived(frameData: ByteArray, width: Int, height: Int, timestamp: Long, frameNumber: Int) {
        ...
        handAnalysisHandler.setAnalysisCallback(object : HandAnalysisHandler.HandAnalysisCallback {
            ...
            override fun onHandDetected(frameNumber: Int, timestamp: Long, handLandmarks: List<HandAnalysisHandler.HandLandmark>) {
                runOnUiThread {
    ...
    private fun startRecording() {
        ...
        try {
            ...
            cameraHandler.setSessionInfo(currentSessionId!!, deviceId)
            ...
            cameraHandler.addTimestampMarker("RECORDING_START", "Session: $currentSessionId")
    ...
    private fun stopRecording() {
        ...
        try {
            ...
            cameraHandler.addTimestampMarker("RECORDING_STOP", "Session: $currentSessionId")
            cameraHandler.triggerCombinedSyncMarker(200) // 200ms flash duration
    ...
    private fun startHandAnalysis() {
        try {
            ...
            if (videoPath != null) {
                ...
                val sessionId = currentSessionId ?: "session_${SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(Date())}"
    ...
    private fun generateRealisticThermalFrame(frameCount: Int): ThermalFrameData {
        ...
        for (y in 0 until height) {
            for (x in 0 until width) {
                ...
                val distanceFromCenter = Math.sqrt(((x - centerX) * (x - centerX) + (y - centerY) * (y - centerY)).toDouble())
        ...
        return ThermalFrameData(
            ...
            width = width,
            height = height,
            minTemp = temperatureMatrix.flatten().minOrNull() ?: 20.0f,
            maxTemp = temperatureMatrix.flatten().maxOrNull() ?: 30.0f,
            ...
            while (isThermalRecording && thermalCameraConnected) {
                try {
                    ...
                    synchronized(thermalFrameBuffer) {
                        thermalFrameBuffer.add(thermalBitmap)
    ...
    private fun saveThermalFrameWithData(
        ...
        try {
            ...
            if (frameNumber % 25 == 0) { // Every second at 25 FPS
                ...
                val avgTemp = thermalData.temperatureMatrix.flatten().average()
                summaryFile.appendText(
