...
class HandAnalysisHandler(private val context: Context) {
    ...
    fun initialize() {
        try {
            ...
            hands?.setResultListener { result ->
                processHandsResult(result)
            }
            hands?.setErrorListener { message, exception ->
                Log.e(TAG, "MediaPipe Hands error: $message", exception)
                analysisCallback?.onAnalysisError("MediaPipe error: $message", System.currentTimeMillis())
            }
     ...
     */
    private suspend fun processFrame(bitmap: Bitmap, frameNumber: Int, timestampMs: Long, resultsWriter: BufferedWriter?) {
        try {
