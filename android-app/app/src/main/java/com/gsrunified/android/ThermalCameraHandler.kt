...
class ThermalCameraHandler(
    ...
    private val usbMonitorListener =
            ...
            for (y in 0 until THERMAL_HEIGHT) {
                for (x in 0 until THERMAL_WIDTH) {
                    ...
                    if (pixelIndex + 1 < rawFrame.size) {
                        ...
                        val rawValue =
                            ((rawFrame[pixelIndex + 1].toInt() and 0xFF) shl 8) or
    ...
    private fun updateThermalDisplay(processedFrame: ThermalProcessedFrame) {
        try {
            ...
            Log.d(
                TAG,
                "Thermal display updated - Temp range: ${processedFrame.minTemp}°C to ${processedFrame.maxTemp}°C",
            )
        } catch (e: Exception) {
