...
class GsrHandler(
    ...
    private val gsrValueText: TextView,
    shimmer: Shimmer? = null,
) {
    private val shimmer: Shimmer by lazy {
    ...
    fun connectToDevice(deviceAddress: String) {
        try {
            shimmer.connect(deviceAddress, "default")
    ...
    fun startStreaming() {
        try {
            if (gsrConnected) {
                ...
                shimmer.setSensorEnabledState(Configuration.Shimmer3.SENSOR_ID.SHIMMER_GSR, true)
                shimmer.setSamplingRateShimmer(128.0) // 128 Hz as specified in requirements
