...
class LSLStreamer:
    ...
    def push_ppg_sample(self, ppg_value: float, heart_rate: float,
        ...
        if 'ppg' in self.streams:
            try:
                if timestamp is None:
                    timestamp = pylsl.local_clock()
                self.streams['ppg'].push_sample(
                    [ppg_value, heart_rate], timestamp)
            except Exception as e:
    ...
    def close_all_streams(self) -> None:
        ...
        for stream_name, outlet in self.streams.items():
            try:
                # LSL outlets are automatically cleaned up when they go out of
                # scope
        ...
        while self.is_receiving:
            for stream_name, inlet in self.inlets.items():
                try:
                    ...
                    self.logger.error(
                        f"Error receiving from {stream_name}: {e}")
