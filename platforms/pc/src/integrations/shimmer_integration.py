...
try:
    ...
    serial = None
    logging.warning(
        "pyserial not available. Shimmer serial functionality will be disabled.")
...
try:
    ...
    bleak = None
    logging.warning(
        "bleak not available. Shimmer Bluetooth functionality will be disabled.")
...
class ShimmerSensor:
    ...
    def connect_serial(self, port: str = None, baudrate: int = 115200) -> bool:
        ...
        try:
            if port is None:
                ...
                if port is None:
                    self.logger.error(
                        "No Shimmer device found on serial ports")
                    return False
    ...
    def connect_bluetooth(self, address: str = None) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(
                    f"Connected to Shimmer sensor via Bluetooth: {address}")
                return True
            else:
                return False
            ...
            self.logger.error(
                f"Error connecting to Shimmer via Bluetooth: {e}")
            return False
    ...
    def _find_shimmer_bluetooth_device(self) -> Optional[str]:
        ...
        # For now, return None (would need to be implemented based on specific
        # requirements)
        return None
    ...
    def _read_serial_data(self) -> None:
        ...
        try:
            ...
            # Adjust size based on packet format
            data = self.serial_connection.read(10)
    ...
    def start_all_streaming(self) -> bool:
        ...
        for device_id, sensor in self.sensors.items():
            if sensor.is_connected:
                ...
                if not started:
                    self.logger.error(
                        f"Failed to start streaming for sensor: {device_id}")
                    success = False
