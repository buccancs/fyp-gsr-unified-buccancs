...
try:
    ...
    logging.warning(
        "PsychoPy not available. Experiment functionality will be disabled.")
            ...
            if keys:
                ...
                if self.response_callback:
                    self.response_callback(
                        keys[0].name, response_data['response_time'])
        ...
        try:
            ...
            if 'stimulus' in trial_config:
                ...
                if stimulus_config['type'] == 'text':
                    stimulus = self.create_text_stimulus(
                        stimulus_config['content'])
                elif stimulus_config['type'] == 'image':
                    stimulus = self.create_image_stimulus(
                ...
                stimulus_data = self.show_stimulus(
                    ...
                    wait_for_response=stimulus_config.get(
                        'wait_for_response',
                        False))
    ...
    def create_emotional_faces(window, image_directory: str) -> List[Dict]:
        ...
        for filename in os.listdir(image_directory):
            if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                # Extract emotion from filename (assuming format:
                # emotion_id.jpg)
                emotion = filename.split(
                    '_')[0] if '_' in filename else 'neutral'
