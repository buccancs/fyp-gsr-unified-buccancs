...
class CalibrationWorker(QThread):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            total_cameras = len(
                [k for k in self.config.keys() if k.endswith('_path') and self.config[k]])
            current_camera = 0
            ...
            if self.config.get('rgb_video_path'):
                ...
                if rgb_images:
                    self.calibrator.calibrate_camera_intrinsics(
                        'rgb_camera', rgb_images)
                current_camera += 1
                self.progress_updated.emit(
                    int(30 + (current_camera / total_cameras) * 40))

            # Process RGB frames directory
            elif self.config.get('rgb_frames_path'):
                self.status_updated.emit("Processing RGB frames...")
                rgb_images = self.calibrator.load_image_sequence(
                    self.config['rgb_frames_path'])
                if rgb_images:
                    self.calibrator.calibrate_camera_intrinsics(
                        'rgb_camera', rgb_images)
                current_camera += 1
            ...
            # Perform stereo calibration if enabled and multiple cameras
            # available
            camera_names = list(self.calibrator.cameras.keys())
            if len(camera_names) >= 2 and self.config.get(
                    'enable_stereo', False):
                ...
                if 'rgb_camera' in camera_names and 'thermal_camera' in camera_names:
                    if self.config.get('rgb_video_path') and self.config.get(
                            'thermal_frames_path'):
                        rgb_images = self.calibrator.extract_frames_from_video(
            ...
            if self.config.get('output_path'):
                ...
                self.calibrator.save_calibration_results(
...
class CalibrationDialog(QDialog):
    ...
    def create_input_tab(self) -> None:
        ...
        self.rgb_video_edit.setPlaceholderText(
            "Select RGB video file (MP4)...")
        ...
        self.rgb_frames_edit.setPlaceholderText(
            "Or select RGB frames directory...")
        ...
        self.thermal_frames_edit.setPlaceholderText(
            "Select thermal frames directory...")
        ...
        self.webcam_video_edit.setPlaceholderText(
            "Select webcam video file...")
        ...
        self.auto_detect_button = QPushButton(
            "🔍 Auto-Detect from Session Folder")
    ...
    def create_advanced_tab(self) -> None:
        ...
        self.output_edit.setPlaceholderText(
            "Select output file for calibration results...")
...
• Better corner detection accuracy
    ...
    def validate_inputs(self) -> bool:
        ...
        has_rgb = bool(self.rgb_video_edit.text()
                       or self.rgb_frames_edit.text())
    ...
    def start_calibration(self) -> None:
        ...
        self.worker.calibration_completed.connect(
