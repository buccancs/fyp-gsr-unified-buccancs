...
from utils.realtime_calibration_feedback import (CalibrationFeedbackVisualizer,
                                                 PatternFeedback,
                                                 PatternQuality,
                                                 RealtimeCalibrationFeedback)
...
class VideoPreviewWidget(QLabel):
    ...
    def update_frame(
        ...
        if self.feedback_overlay and feedback:
            frame = CalibrationFeedbackVisualizer.draw_feedback_overlay(
                frame, feedback)
...
class LiveCalibrationWorker(QThread):
    ...
    def __init__(self, video_source: str, pattern: CalibrationPattern) -> None:
        ...
        self.feedback_system = RealtimeCalibrationFeedback(
            pattern, self.logger)
        self.cap = None
...
class CalibrationQualityWidget(QWidget):
    ...
    def update_feedback(self, feedback: PatternFeedback) -> None:
        ...
        self.quality_label.setStyleSheet(
            f"font-weight: bold; font-size: 14px; color: {quality_color};")
...
class LiveCalibrationDialog(QDialog):
    ...
    def _create_video_panel(self) -> QWidget:
        ...
        self.overlay_checkbox.toggled.connect(
            self.video_preview.toggle_feedback_overlay)
    ...
    def _create_control_panel(self) -> QWidget:
        ...
        self.run_calibration_button = QPushButton(
            "🔧 Run Calibration on Frames")
        self.run_calibration_button.clicked.connect(
            self.run_calibration_on_frames)
    ...
    def on_frame_processed(self, frame: np.ndarray, feedback: PatternFeedback) -> None:
        ...
        if feedback:
            ...
            if (self.recording_session and feedback.detected and feedback.quality in [
                    PatternQuality.EXCELLENT, PatternQuality.GOOD]):

                # Avoid capturing too many frames too quickly
                if (not self.captured_frames or time.time() -
                        self.captured_frames[-1]['timestamp'] > 2.0):
    ...
    def export_captured_frames(self) -> None:
        ...
        if not self.captured_frames:
            QMessageBox.warning(
                self, "No Frames", "No frames have been captured yet.")
            return
        ...
        try:
            ...
            metadata = {
                ...
                'grid_size': [
                    self.grid_width_spin.value(),
                    self.grid_height_spin.value()],
    ...
    def run_calibration_on_frames(self) -> None:
        ...
        try:
            ...
            # Run calibration
            results = calibrator.calibrate_camera_intrinsics(
                'live_camera', frames)
...
{np.array(results['distortion_coefficients'])}
        ...
        save_button.clicked.connect(
            lambda: self._save_calibration_results(results))
