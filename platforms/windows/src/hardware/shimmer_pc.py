...
class ShimmerDataThread(QThread):
    ...
    This thread runs the blocking pyshimmer.read_data() method to avoid
    freezing the UI.
    """
    ...
    def __init__(self, shimmer_device, parent=None):
    ...
    def run(self):
        ...
        while self.running:
            try:
                if self.shimmer_device and self.shimmer_device.is_connected():
                    # Read data from Shimmer (this is blocking)
                    ...
                    # If not connected, wait longer before checking again
                    self.msleep(100)
...
class ShimmerPC(PCConnectedSensor):
    ...
    def __init__(self, com_port=None, parent=None):
        ...
        if pyshimmer is None:
            self.logger.error("pyshimmer library not available")
    ...
    def connect(self):
        ...
        if pyshimmer is None:
            self.error.emit("pyshimmer library not available")
            return
        
        try:
            self.logger.info(f"Connecting to Shimmer on {self.com_port}")
    ...
    def start_streaming(self):
        ...
        try:
            ...
            if self.shimmer_device.start_streaming():
                ...
                self.data_thread = ShimmerDataThread(self.shimmer_device, self)
                ...
                self.error.emit("Failed to start Shimmer streaming")
