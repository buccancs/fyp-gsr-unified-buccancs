    ...
    def __init__(self, pattern_type: str, grid_size: Tuple[int, int],
        ...
        if self.pattern_type == 'charuco':
            ...
            self.board = cv2.aruco.CharucoBoard(
                grid_size, square_size, self.marker_size, self.aruco_dict
            )
        elif self.pattern_type == 'chessboard':
            # Create object points for chessboard
            self.object_points = np.zeros(
                (grid_size[0] * grid_size[1], 3), np.float32)
            self.object_points[:, :2] = np.mgrid[0:grid_size[0],
                                                 0:grid_size[1]].T.reshape(-1, 2)
...
class CameraCalibrator:
        ...
        for i, image in enumerate(images):
            ...
            if success:
                if self.pattern.pattern_type == 'chessboard':
                    ...
                    # For ChArUco, we need to get the object points for
                    # detected corners
                ...
                self.logger.debug(
                    f"Pattern detected in image {i + 1}/{len(images)}")
            else:
                self.logger.debug(
                    f"Pattern not detected in image {i + 1}/{len(images)}")
    ...
    def calibrate_stereo_extrinsics(self,
                                    camera1_name: str,
                                    camera2_name: str,
        ...
        self.logger.info(
            f"Starting stereo calibration between {camera1_name} and {camera2_name}")
        ...
        for i, (img1, img2) in enumerate(zip(images1, images2)):
            ...
            if success1 and success2:
                ...
                self.logger.debug(
                    f"Pattern detected in both images {i + 1}/{len(images1)}")
        ...
        self.logger.info(
            f"Stereo calibration completed between {camera1_name} and {camera2_name}")
...
def main():
    ...
    try:
        ...
        if args.rgb_video:
            ...
            rgb_images = calibrator.extract_frames_from_video(
                args.rgb_video, args.max_frames)
            if rgb_images:
        ...
        if args.thermal_frames:
            ...
            thermal_images = calibrator.load_image_sequence(
                args.thermal_frames)
            if thermal_images:
                calibrator.calibrate_camera_intrinsics(
        ...
        if args.webcam_video:
            ...
            webcam_images = calibrator.extract_frames_from_video(
                args.webcam_video, args.max_frames)
            if webcam_images:
                calibrator.calibrate_camera_intrinsics(
        ...
        if len(camera_names) >= 2:
            ...
            if 'rgb_camera' in camera_names and 'thermal_camera' in camera_names:
                if args.rgb_video and args.thermal_frames:
                    ...
                    logger.warning(
                        "Stereo calibration requires synchronized images. "
