...
class DeviceManager:
    ...
    def __init__(self, session_manager=None):
        ...
        self.clients = {}  # device_id -> {"socket": socket, "info": device_info, "last_heartbeat": timestamp}
        self.client_threads = {}
    ...
    def _handle_client(self, client_socket, addr):
        ...
        try:
            ...
            device_id = device_registration.get("device_id", f"unknown_{addr[0]}_{addr[1]}")
            ...
            if self.session_manager and self.session_manager.is_session_active():
                try:
                    device_dir = self.session_manager.register_device(device_id, device_info)
                    logger.info(f"Device {device_id} registered to session directory: {device_dir}")
                except Exception as e:
                    logger.error(f"Failed to register device with session manager: {e}")

            # Send registration confirmation with time sync
            self._send_message(device_id, {
            ...
            while self.is_running:
                try:
                    ...
                    if time.time() - self.clients[device_id]["last_heartbeat"] > self.heartbeat_timeout:
    ...
    def _handle_data_stream(self, device_id: str, message: Dict[str, Any]):
        ...
        logger.debug(f"Data stream from {device_id}: {data.get('data_type', 'unknown')}")
    ...
    def _send_message(self, device_id: str, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(f"Sent message to {device_id}: {message.get('command', 'unknown')}")
            return True
    ...
    def send_command(self, device_id: str, command: str, data: Optional[Dict[str, Any]] = None) -> bool:
    ...
    def send_command_to_all(self, command: str, data: Optional[Dict[str, Any]] = None) -> int:
    ...
    def send_command_to_device(self, device_id: str, command: str, data: Optional[Dict[str, Any]] = None) -> bool:
