...
class GSRHandler:
    ...
    def discover_shimmer_devices(self) -> List[str]:
        ...
        try:
            ...
            for port in ports:
                ...
                if port.description and ("shimmer" in port.description.lower() or 
                                       "bluetooth" in port.description.lower()):
                    shimmer_ports.append(port.device)
                    logger.info(f"Found potential Shimmer device: {port.device} - {port.description}")
    ...
    def _start_serial_capture(self) -> bool:
        ...
        try:
            ...
            self.capture_thread = threading.Thread(target=self._serial_capture_loop, daemon=True)
    ...
    def _start_lsl_capture(self) -> bool:
        ...
        try:
            ...
            self.is_capturing = True
            self.start_time = time.time()
            self.capture_thread = threading.Thread(target=self._lsl_capture_loop, daemon=True)
            self.capture_thread.start()
    ...
    def _start_simulation_capture(self) -> bool:
        ...
        self.is_capturing = True
        self.start_time = time.time()
        self.capture_thread = threading.Thread(target=self._simulation_capture_loop, daemon=True)
        self.capture_thread.start()
                        ...
                        try:
                            ...
                            if len(parts) >= 2:
                                ...
                                data = {
                                    ...
                                    'resistance': 1.0 / gsr_value if gsr_value > 0 else 0,
                                    'unit': 'microsiemens',
        ...
        try:
            while self.is_capturing:
                try:
                    ...
                    baseline = 15.0
                    slow_drift = 2.0 * (0.5 + 0.5 * random.random()) * (1 + 0.3 * random.random())
                    ...
                    conductance = max(0.1, baseline + slow_drift + heart_rate + noise + scr)
