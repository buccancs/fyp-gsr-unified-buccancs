...
from PySide6.QtWidgets import (
    ...
    QPushButton, QGroupBox, QTextEdit, QProgressBar, QSlider,
...
class GSRPlotWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.time_range_combo.addItems(["10 seconds", "30 seconds", "1 minute", "5 minutes", "All"])
    ...
    def update_plot(self):
        ...
        for i, (device_id, data) in enumerate(self.gsr_data.items()):
            ...
            self.ax.plot(times, values, color=color, label=f"Device {device_id}", linewidth=1.5)
...
class ThermalVisualizationWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.thermal_display_1.setStyleSheet("border: 1px solid gray; background-color: #f0f0f0;")
        ...
        self.thermal_display_2.setStyleSheet("border: 1px solid gray; background-color: #f0f0f0;")
    ...
    def update_thermal_data(self, device_id: str, thermal_data: Dict):
        ...
        if device_id.endswith("1") or len(self.thermal_data) == 1:
            self.update_thermal_display(self.thermal_display_1, self.temp_info_1, thermal_data)
        else:
            self.update_thermal_display(self.thermal_display_2, self.temp_info_2, thermal_data)
            
    def update_thermal_display(self, display_label: QLabel, info_label: QLabel, thermal_data: Dict):
    ...
    def update_analysis(self):
        ...
        try:
            ...
            if latest_value is not None:
                ...
                timestamps, values = self.gsr_handler.get_data_array(device_id, duration_seconds=10.0)
                ...
                if len(values) > 0:
                    ...
                    quality = max(0, min(100, 100 - (std_val * 10)))  # Lower std = higher quality
        ...
        try:
            ...
            for device_id in device_status.keys():
                ...
                if analysis.get('status') == 'analyzed':
                    ...
                    result_text += f"  Artifacts: {analysis.get('artifact_percentage', 0):.1f}%\n"
                    
                    analysis_results.append(result_text)
                    
            if analysis_results:
                timestamp = datetime.now().strftime("%H:%M:%S")
                full_result = f"[{timestamp}] Analysis Results:\n" + "\n".join(analysis_results) + "\n"
