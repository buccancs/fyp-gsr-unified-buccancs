...
class RealtimeCalibrationFeedback:
    ...
    def __init__(self, pattern: CalibrationPattern, logger: logging.Logger = None):
        ...
        self.feedback_callback: Optional[Callable[[PatternFeedback], None]] = None
        
    def set_feedback_callback(self, callback: Callable[[PatternFeedback], None]):
    ...
    def process_frame(self, frame: np.ndarray, timestamp: float = None) -> Optional[PatternFeedback]:
        """
    ...
    def _analyze_frame(self, frame: np.ndarray, timestamp: float) -> PatternFeedback:
        """
        ...
        if not detected:
            feedback.recommendations = self._get_detection_recommendations(gray)
            return feedback
            
        # Calculate pattern metrics
        feedback.corners_count = len(corners) if corners is not None else 0
        feedback.pattern_area_ratio = self._calculate_pattern_area_ratio(corners, frame.shape)
        feedback.sharpness_score = self._calculate_sharpness(gray)
        feedback.lighting_score = self._calculate_lighting_quality(gray)
        feedback.angle_score = self._calculate_angle_score(corners, frame.shape)
    ...
    def _calculate_pattern_area_ratio(self, corners: np.ndarray, image_shape: Tuple[int, int, int]) -> float:
    ...
    def _calculate_angle_score(self, corners: np.ndarray, image_shape: Tuple[int, int, int]) -> float:
        ...
        if self.pattern.pattern_type == 'chessboard':
            ...
            expected_ratio = self.pattern.grid_size[0] / self.pattern.grid_size[1]
    ...
    def _determine_quality(self, feedback: PatternFeedback) -> PatternQuality:
        ...
        sharpness_score = min(100, feedback.sharpness_score / self.min_sharpness_threshold * 100)
    ...
    def _generate_recommendations(self, feedback: PatternFeedback) -> List[str]:
        ...
        if feedback.pattern_area_ratio < self.min_pattern_area_ratio:
            recommendations.append("Move the calibration pattern closer to the camera")
        elif feedback.pattern_area_ratio > self.max_pattern_area_ratio:
            recommendations.append("Move the calibration pattern further from the camera")
            
        # Sharpness recommendations
        if feedback.sharpness_score < self.min_sharpness_threshold:
            recommendations.append("Ensure the pattern is in focus and avoid motion blur")
            
        # Lighting recommendations
        if feedback.lighting_score < self.min_lighting_score:
            recommendations.append("Improve lighting conditions for better contrast")
            
        # Angle recommendations
        if feedback.angle_score < 70:
            recommendations.append("Try to position the pattern more perpendicular to the camera")
            
        # Quality-specific recommendations
        if feedback.quality == PatternQuality.POOR:
            recommendations.append("Consider repositioning the pattern for better detection")
        elif feedback.quality == PatternQuality.FAIR:
            recommendations.append("Good detection - try to improve positioning for better quality")
        elif feedback.quality in [PatternQuality.GOOD, PatternQuality.EXCELLENT]:
            recommendations.append("Excellent pattern detection - capture this position!")
            
        return recommendations
        
    def _get_detection_recommendations(self, gray_image: Optional[np.ndarray]) -> List[str]:
        ...
        if gray_image is not None:
            ...
            if mean_brightness < 50:
                recommendations.append("Image appears too dark - increase lighting")
            elif mean_brightness > 200:
                recommendations.append("Image appears too bright - reduce lighting or exposure")
...
class CalibrationFeedbackVisualizer:
    ...
    @staticmethod
    def _draw_status_box(frame: np.ndarray, feedback: PatternFeedback, color: Tuple[int, int, int]):
        ...
        cv2.putText(frame, quality_text, (20, y_offset), font, font_scale, color, thickness)
        y_offset += line_height
        
        # Detection status
        detection_text = f"Detected: {'YES' if feedback.detected else 'NO'}"
        cv2.putText(frame, detection_text, (20, y_offset), font, font_scale, (255, 255, 255), thickness)
        y_offset += line_height
        
        if feedback.detected:
            # Metrics
            metrics_text = f"Area: {feedback.pattern_area_ratio:.2f} | Sharpness: {feedback.sharpness_score:.0f}"
            cv2.putText(frame, metrics_text, (20, y_offset), font, 0.5, (255, 255, 255), thickness)
            y_offset += line_height
            
            confidence_text = f"Confidence: {feedback.confidence:.2f}"
            cv2.putText(frame, confidence_text, (20, y_offset), font, font_scale, (255, 255, 255), thickness)
    ...
    def _draw_recommendations(frame: np.ndarray, recommendations: List[str]):
        ...
        cv2.rectangle(frame, (10, y_start), (width - 10, height - 10), (255, 255, 255), 1)
        ...
        cv2.putText(frame, "Recommendations:", (20, y_start + 25), font, 0.6, (255, 255, 0), thickness)
