...
def test_synthetic_calibration():
    ...
    try:
        ...
        for i in range(0, img_size[0], square_size):
            for j in range(0, img_size[1], square_size):
                if ((i // square_size) + (j // square_size)) % 2 == 0:
                    cv2.rectangle(img, (i, j), (i + square_size, j + square_size), (255, 255, 255), -1)
        ...
        if success:
            ...
            print("! Pattern detection failed on synthetic image (expected for simple test)")
...
def test_gui_imports():
    ...
    try:
        ...
        print(f"! GUI components not available (this is OK for headless systems): {e}")
