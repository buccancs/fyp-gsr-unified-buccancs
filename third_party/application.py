...
try:
    ...
    from PySide6.QtCore import QObject, QThread, Slot as pyqtSlot
...
class GSRRGBTApplication(QMainWindow):
    ...
    def __init__(self, config: Optional[Config] = None) -> None:
        ...
        if not PYSIDE6_AVAILABLE:
            raise RuntimeError("PySide6 is required for GUI functionality")
    ...
    def start_recording(self, subject_id: str) -> None:
        ...
        try:
            if self.state != ApplicationState.IDLE:
                raise CaptureError(
                    f"Cannot start recording in state: {self.state.name}"
                )
        ...
        try:
            if self.state != ApplicationState.RECORDING:
                self._logger.warning(
                    f"Stop recording called in state: {self.state.name}"
                )
                return
...
def main(config_path: Optional[str] = None) -> int:
    ...
    """
    if not PYSIDE6_AVAILABLE:
        print("Error: PySide6 is required to run the GUI application")
        return 1
